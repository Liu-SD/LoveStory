...
$(function () {
    ...
    for (entry of desc) {
        if (typeof entry == "string") {
            ul_id = `GallaryUl${ul_count}`
            ul_count += 1
            $("#gallary").append(`
<div class="GITheWallBtn row">
<button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#${ul_id}" aria-expanded="false" aria-controls="${ul_id}">
    ${entry}
</button>
</div>
<div class="GITheWall collapse show row">
    <ul id="${ul_id}">
</div>
`)
            ul = $(`#${ul_id}`)
        ...
        filename = entry[0]
        description = entry[1]
        ul.append(`<li data-contenttype="image" data-href="${filename}" , data-text="${description}">
    <img src="${filename}" />
</li>`)
    }
