  ...
  var GI_TW_ID = 0,
    TheWall = function ($el, myOptions) {
      ...
      var execCallback = function (callback, arg) {
         ...
         * @return { Object } jquery deferred object
         */
        _loadImage = function (src, text) {
          var dfr = new $.Deferred(),
            img = new Image();
          img.onload = function () {
            self.$expanderInner.html('<div class="GI_TW_fullimg"><img src="' + src + '" /><div class="GI_TW_text">' + text + '</div></div>');
            dfr.resolve();
            img = null;
          };
      ...
      this.loadInnerContents = function () {
        var $a = this.$selectedli.find('a'),
          ...
          text = this.selectedLiData.text || $a.attr('text');
        isLoading = true;
